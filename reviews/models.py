...
class Review(models.Model):
    ...
    product = models.ForeignKey('ProductObject.ProductObject',on_delete=models.CASCADE)
...
def calculate_avg_review(sender,instance,created,**kwargs):
    if instance.status == ReviewStatusType.accepted.value:
        ...
        average_rating = Review.objects.filter(product=product, status=ReviewStatusType.accepted).aggregate(Avg('rate'))['rate__avg']
